...
class FilterCondition implements Verbosable {
    ...
    public function isMet(FilterDependencyProvider $provider): bool {
        ...
        if ($this->getLogger()) $this->getLogger()->tock();
     ...
     * @since 1.11.0
     */
    public function forEachCommand(?callable $callback) {
        if (!$callback) return;
     ...
     * @since 1.11.0
     */
    protected function addConditionSubjectItems(bool $and, array &$fieldKeyItemMap, FilterCondition $condition) {
     ...
     *
     * @param bool     $and             True if the operator to be used to add the subject items is "and". Otherwise,
     *                                  false.
     * @param array    $fieldKeyItemMap Key-value pairs where keys are field keys and the values are string arrays
     *                                  containing dot keys or object arrays
     * @param string[] $subjectItems    The items that will be added to $fieldKeyItemMap
     * @param string   $fieldKey        Field key under which the subject items will be added to $fieldKeyItemMap
     * @since 1.11.0
     */
    protected function addSubjectItems(bool $and, array &$fieldKeyItemMap, array $subjectItems, string $fieldKey) {
    ...
    public static function fromOptions(?array $options, ?bool $verbose = null): ?FilterCondition {
        ...
        $filterCondition = new static($operator, $items);
