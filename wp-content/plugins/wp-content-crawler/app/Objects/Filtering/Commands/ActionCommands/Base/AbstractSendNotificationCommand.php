...
abstract class AbstractSendNotificationCommand extends AbstractActionCommand implements NeedsBot {
    ...
    protected function createViews(): ?ViewDefinitionList {
        $infoUseShortCodes = 'You can use the short codes shown at the top of the command container to include certain 
            information into the template.';

        $infoEmptyTemplate = 'If you leave this empty, a default template will be used. However, the default template
            will not include specific information that you can use to differentiate notifications. Hence, it is 
            recommended that you enter a descriptive value.';
        ...
        return (new ViewDefinitionList())
            ...
            ->add((new ViewDefinition(TextAreaWithLabel::class))
                ...
                ->setVariable(ViewVariableName::INFO,  _wpcc("Title of the notification. {$infoUseShortCodes} 
                        {$infoEmptyTemplate}"))
            ...
            ->add((new ViewDefinition(TextAreaWithLabel::class))
                ...
                ->setVariable(ViewVariableName::INFO,  _wpcc("Enter the message that will be sent as a 
                    notification. {$infoUseShortCodes} {$infoEmptyTemplate}"))
            ...
            ->add((new ViewDefinition(MaxLengthInputWithLabel::class))
                ...
                ->setVariable(ViewVariableName::INFO,  sprintf(_wpcc('A text that will be used to identify this 
    ...
    protected function getLastNotificationDate(): ?DateTime {
        ...
        $date = DateTime::createFromFormat(Environment::mysqlDateFormat(), $lastDateString);
        return $date ?: null;
    }
    ...
    protected function createNotificationInfo(): NotificationInfo {
        $titleTemplate   = $this->getOption(InputName::NOTIFICATION_TITLE_TEMPLATE);
        $messageTemplate = $this->getOption(InputName::NOTIFICATION_MESSAGE_TEMPLATE);
        ...
        return new NotificationInfo(
            $prefix . $this->replaceShortCodes($shortCodeMap, $titleTemplate),
            $prefix . $this->replaceShortCodes($shortCodeMap, $messageTemplate)
        );
    }
    ...
    protected function getShortCodeReplacementMap(): array {
        return [
            ...
            CommandShortCodeName::SITE_EDIT_URL => function() {
                $siteId = $this->getBot() ? $this->getBot()->getSiteId() : null;
                if (!$siteId) return "#";
            ...
            CommandShortCodeName::SITE_ID => $this->getBot() ? $this->getBot()->getSiteId() : "",
        ];
    }
     ...
     * @since 1.11.0
     */
    protected function addLogMessage(string $message, bool $addToLogger, bool $addToInformer) {
    ...
    public function getIdentifierOption(): ?string {
        $id = $this->getOption(InputName::NOTIFICATION_ID);
