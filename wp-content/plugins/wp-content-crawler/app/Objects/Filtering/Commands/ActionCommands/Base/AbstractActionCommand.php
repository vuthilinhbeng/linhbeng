...
abstract class AbstractActionCommand extends AbstractBaseCommand {
    ...
    /**
     * @var string[]|null A sequential array of dot keys. This command will be executed only for these keys if
    ...
    /** @var bool True if only the allowed subjects should be used. Otherwise, false. */
    ...
    /**
     * @param string[]|null $allowedSubjects See {@link allowedSubjects}
     * @return $this
     * @since 1.11.0
     */
    ...
    /**
     * @return string[]|null See {@link allowedSubjects}
     * @since 1.11.0
     */
    ...
    public function execute() {
        ...
        if ($this->isVerbose()) $this->logger = (new ActionCommandLogger())->tick();
    ...
    protected function doExecute() {
        ...
        if ($logger) $logger->setOnlyAllowed($this->onlyAllowed);
        ...
        if ($this->shouldReassignNewValues()) {
            $this->reassignFieldValues($newSubjectValues);
        }
    }
     ...
     *                                method. The only difference is that the values are the return values retrieved from
     *                                {@link onExecute()}.
     * @since 1.11.0
     */
    protected function reassignFieldValues(array $newSubjectValues): void {
        ...
        if ($this->isPropertyCalculated()) {
            ...
            if (is_a($property, AbstractActionProperty::class)) {
        ...
        foreach($newSubjectValues as $dotKey => $newSubjectValue) {
            if ($logger) $logger->addModifiedSubjectItem($newSubjectValue);
     ...
     * @since 1.11.0
     */
    protected function setFieldValue(?string $dotKey, $newValue, $oldValue) {
        ...
        if ($isOutputTypeValid === false && $newValue !== null) {
            ...
            if ($preparedNewValue === null) {
                ...
                Informer::addError(sprintf(
                    ...
                    $this->getName(),
                    $newValue === null ? 'null' : implode(', ', (array) $valueTypeService->getTypeOf($newValue)),
    ...
    protected function isOutputTypeValid($newValue, $oldValue): ?bool {
        ...
        if (is_object($oldValue)) {
            ...
            if ($cls1 !== $cls2 && !array_intersect($cls1Parents, $cls2Parents)) {
                return false;
            }
        }
    ...
    protected function castNewValue($newValue, $oldValue) {
        ...
        if ($oldValue !== null) {
            ...
            if ($oldValueType !== null && $field->containsDataType($oldValueType)) {
                return $valueTypeService->castTo($this, $newValue, $oldValueType);
            }
        }
