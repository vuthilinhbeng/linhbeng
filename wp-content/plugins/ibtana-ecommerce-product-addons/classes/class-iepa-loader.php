...
if ( ! class_exists( 'IEPA_Loader' ) ) {
	...
	final class IEPA_Loader {
		...
		public function __construct() {
			...
			// Upsell Script
			add_action( 'admin_enqueue_scripts', array( $this, 'iepa_admin_enqueue_upsell_scripts' ), 11 );
		}


		function iepa_admin_enqueue_upsell_scripts( $hooks ) {
      if ( 'nav-menus.php' == $hooks ) {
				...
				if ( !$iepa_key_arr_license_status ) {
					wp_enqueue_style( 'iepa-upsell-css', IEPA_PLUGIN_URI . '/addons/ibtana-mega-menu/css/iepa-upsell.css', array(), IEPA_VERSION );
	        wp_enqueue_script( 'iepa-upsell', IEPA_PLUGIN_URI . '/addons/ibtana-mega-menu/js/iepa-upsell.js', array( 'jquery' ), IEPA_VERSION, true );
					wp_localize_script(
						'iepa-upsell',
						'iepa_upsell_obj',
						array(
							'upsell_image'	=>	IEPA_PLUGIN_URI . '/IEPA_Whizzie/assets/img/ibtana-mega-menu-display-image.png'
						)
					);
				}
		...
		public function iepa_load_plugin() {
			...
			if ( !self::$iepa_loading_errors ) {
				...
				if ( file_exists( IEPA_DIR . 'addons/ibtana-mega-menu/iepa-mega-menu.php' ) && $iepa_key_arr_license_status ) {
					require_once IEPA_DIR . 'addons/ibtana-mega-menu/iepa-mega-menu.php'; // Mega Menu
				}


				if ( file_exists( IEPA_DIR . 'addons/ibtana-variation-swatches/iepa-variation-swatches.php' ) ) {
					require_once IEPA_DIR . 'addons/ibtana-variation-swatches/iepa-variation-swatches.php';
				}


				if ( file_exists( IEPA_DIR . 'addons/ibtana-ajax-search/iepa-ajax-search.php' ) ) {
					require_once IEPA_DIR . 'addons/ibtana-ajax-search/iepa-ajax-search.php';
				}
